...
class ResNetSmallNoFC(ResNet):
    def __init__(self, block, layers, num_classes=200, zero_init_residual=False,
        ...
        self.base_width = width_per_group
        # color + depth = 4
        self.conv1 = nn.Conv2d(input_channels, self.inplanes, kernel_size=5, stride=2, padding=1,
                               bias=False)
        self.bn1 = norm_layer(self.inplanes)
        self.relu = nn.ReLU(inplace=True)
        self.maxpool = nn.MaxPool2d(kernel_size=3, stride=2, padding=1)
...
def resnet10(pretrained=False, progress=True, **kwargs):
    ...
    """
    return _resnet('resnet10', Bottleneck, [1, 1, 1, 1], pretrained, progress,
                   **kwargs)
