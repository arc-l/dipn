...
print(
    'Average %% action efficiency: %2.1f' %
    (100 *
     np.mean(
         np.divide(
             float(num_obj_complete),
             num_actions_before_completion[valid_clearance]))))
print('Average grasp to push ratio: %2.1f' % (np.mean(grasp_to_push_ratio[valid_clearance]) * 100))

print('\nnew')
print('num_actions_before_completion valid', num_actions_before_completion[valid_clearance])
print('num_actions_before_completion valid', num_actions_before_completion)

print(grasp_success_rate * 100)
print('Average %% grasp success per clearance: %2.1f' % (np.mean(grasp_success_rate) * 100))
all_eff = np.divide(float(num_obj_complete), num_actions_before_completion)
all_eff[all_eff>1] = 1
print('Average %% action efficiency before: %2.1f' % (100 * np.mean(all_eff)))
num_actions_before_completion[num_actions_before_completion > num_obj_complete * 3] = num_obj_complete * 3
all_eff = np.divide(float(num_obj_complete), num_actions_before_completion)
all_eff[all_eff>1] = 1
print('Average %% action efficiency after: %2.1f' % (100 * np.mean(all_eff)))
