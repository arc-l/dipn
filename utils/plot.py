...
for session_idx in range(len(session_directories)):
    ...
    if len(session_directories) > 1 and session_idx + 1 == len(session_directories):
        ...
        plt.plot(range(0, max_iteration + 2000), grasp_success, color=color, linewidth=3) # color='blue', linewidth=3)
        # plt.fill_between(range(0, max_iteration), push_then_grasp_success, 0, color=color, alpha=0.5)
        plt.plot(range(0, max_iteration + 2000), push_then_grasp_success, dashes=[8,7], color=color, linewidth=3, alpha=1, dash_capstyle='round', dash_joinstyle='round',label='_nolegend_') # color='blue', dashes=[5,5], linewidth=2, dash_capstyle='butt')
