...
max_plot_iteration = 3000 # Maximum number of training steps to report performance
...
ax.set_xlabel('Number of training steps', fontsize = FONT_SIZE)
...
for session_idx in range(len(session_directories)):
    ...
    if len(session_directories) > 1 and session_idx + 1 == len(session_directories):
        grasp_success_pad = np.zeros((max_iteration + 2000))
        push_then_grasp_success_pad = np.zeros((max_iteration + 2000))
        grasp_success_pad[2119:] = grasp_success[:1000]
        push_then_grasp_success_pad[2119:] = push_then_grasp_success[:1000]
        grasp_success = grasp_success_pad
        push_then_grasp_success = push_then_grasp_success_pad
        # Plot grasp information
        ax.plot(range(0, max_iteration + 2000), grasp_success, color=color) # color='blue', linewidth=3)
        # plt.fill_between(range(0, max_iteration), push_then_grasp_success, 0, color=color, alpha=0.5)
        ax.plot(range(0, max_iteration + 2000), push_then_grasp_success, dashes=[8,7], color=color, alpha=1, dash_capstyle='round', dash_joinstyle='round',label='_nolegend_') # color='blue', dashes=[5,5], linewidth=2, dash_capstyle='butt')
        ...
        # Plot grasp information
        ax.plot(range(0, max_iteration), grasp_success, color=color, ) # color='blue', linewidth=3)
        # plt.fill_between(range(0, max_iteration), push_then_grasp_success, 0, color=color, alpha=0.5)
        ax.plot(range(0, max_iteration), push_then_grasp_success, dashes=[8,7], color=color, alpha=1, dash_capstyle='round', dash_joinstyle='round',label='_nolegend_') # color='blue', dashes=[5,5], linewidth=2, dash_capstyle='butt')
        # plt.plot(range(0, max_iteration), grasp_to_push_ratio, dashes=[1,5], color=color, linewidth=2, alpha=0.5, dash_capstyle='round', dash_joinstyle='round') # color='blue', dashes=[5,5], linewidth=2, dash_capstyle='butt')
        legend.append(session_directories[session_idx])

ax.legend(legend, loc='lower right', fontsize=LEGEND_FONT_SIZE)
fig.savefig(os.path.join(OUTDIR, "simulation-learning-curve.pdf"), bbox_inches="tight", pad_inches=0.05)
plt.show()
